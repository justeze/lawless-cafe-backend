...
const menuModel = {
    getAllMenu: (query) => { // INI DIAPUS KARENA UDAH ADA YANG PAGINATED
        return new Promise((resolve, reject) => {
            let queryString = ""
            if (query.nama_produk === undefined) {
                queryString = `SELECT produk.id, produk.nama_produk, produk.harga_produk, produk.gambar_produk, kategori.kategori FROM produk JOIN kategori ON produk.id_kategori=kategori.id`;
            } else {
                queryString = `SELECT produk.id, produk.nama_produk, produk.harga_produk, produk.gambar_produk, kategori.kategori FROM produk JOIN kategori ON produk.id_kategori=kategori.id WHERE nama_produk LIKE '%${query.nama_produk}%'`;
            }
            // const queryString = `SELECT produk.id, produk.nama_produk, produk.harga_produk, produk.gambar_produk, kategori.kategori FROM produk JOIN kategori ON produk.id_kategori=kategori.id WHERE nama_produk LIKE '%${query.nama_produk}%'`;
